...
class ParkingSpot {
  ...
  factory ParkingSpot.fromJson(Map<String, dynamic> json) {
    ...
    final availableTypes = availableTypesStr.isNotEmpty
        ? availableTypesStr.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList()
        : <String>[];
    ...
    return ParkingSpot(
      ...
      imageUrl: parkingUser['image_url'] ??
...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  final String _googleMapsApiKey = 'YOUR_API_KEY_HERE'; // Replace with your actual API key
  ...
  Future<void> _fetchParkingSpots() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          setState(() {
            _parkingSpots = List<Map<String, dynamic>>.from(data)
  ...
  void _useDummyData() {
    ...
    setState(() {
      _parkingSpots = dummyData.map((spot) => ParkingSpot.fromJson(spot)).toList();
      _addMarkers();
      _isLoading = false;
    });
  }
    ...
    for (final spot in _parkingSpots) {
      _markers.add(Marker(
        ...
        infoWindow: InfoWindow(
          title: spot.name,
          snippet: '${spot.availableSlots} spots - ₹${spot.hourlyRate.toInt()}/hr',
        ),
  ...
  void _updatePolylines() {
    if (_alternativeRoutes.isEmpty || _selectedRouteIndex >= _alternativeRoutes.length) return;

    setState(() {
      _polylines.clear();
      _polylines.add(Polyline(
        polylineId: const PolylineId('route'),
        points: _alternativeRoutes[_selectedRouteIndex]['polyline'] as List<LatLng>,
        width: 5,
        color: Colors.blue,
      ));
    });
  ...
  void _fitPolylineToBounds() {
    if (_alternativeRoutes.isEmpty || _selectedRouteIndex >= _alternativeRoutes.length || mapController == null) return;

    final points = _alternativeRoutes[_selectedRouteIndex]['polyline'] as List<LatLng>;
                ...
                child: Column(
                  ...
                  children: [
                    Wrap(
                      spacing: 4,
                      runSpacing: 4,
                      children: spot.availableTypes.map((type) =>
                          Container(
                    ...
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        Icon(Icons.local_parking, size: 16, color: Colors.grey.shade700),
                        const SizedBox(width: 4),
                        Text(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ParkingDetailPage(parkingSpot: spot.data),
                              ),
                            );
                          },
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => ParkingDetailPage(parkingSpot: _infoWindowSpot!.data),
                        ),
                      );
                    },
                    style: ElevatedButton.styleFrom(
  ...
  Future<void> _searchPlaces(String input) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          setState(() {
            _autocompletePredictions = List<Map<String, dynamic>>.from(data['predictions']);
    ...
    return Scaffold(
      body: Stack(
        children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _isLoading
                        ...
                        itemBuilder: (context, index) {
                          return _buildParkingSpotCard(_parkingSpots[index], isHorizontal: true);
                        },
                      ),
                    ),
                  ],
          ...
          if (isDesktop && _mapMode == "directions" && _alternativeRoutes.isNotEmpty)
            Positioned(
              top: MediaQuery.of(context).padding.top + 80,
              left: 16,
              bottom: 16,
                    ...
                    children: [
                      GestureDetector(
                        ...
                        onVerticalDragEnd: (details) {
                          if (_draggableScrollableController.isAttached) {
                            final closestPosition = _getClosestSnapPosition(_draggableScrollableController.size);
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        Icon(
                                          _mapMode == "directions" ? Icons.directions : Icons.local_parking,
                                          color: Colors.blue,
                                        ),
                                        const SizedBox(width: 8),
                                        Text(
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 8),
                                        Row(
                                          children: _snapPositions.map((position) {
                                            final isActive = _draggableScrollableController.isAttached &&
                                                (_draggableScrollableController.size - position).abs() < 0.05;
                                            return Container(
                                              width: 8,
                                              height: 8,
